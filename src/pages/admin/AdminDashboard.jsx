...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    document.title = 'Espace OA — Gestion'
    ...
    <div>
      ...
      {dayResas.length>0 ? (
        <div className="day-resas">
          {pageResas.map(r => {
            const rd=getRDates(r); const dateLabel=rd.length<=1?new Date((rd[0]||r.date)+'T12:00:00').toLocaleDateString('fr-FR',{day:'2-digit',month:'long'}):(new Date(rd[0]+'T12:00:00').toLocaleDateString('fr-FR',{day:'2-digit',month:'short'})+' → '+new Date(rd[rd.length-1]+'T12:00:00').toLocaleDateString('fr-FR',{day:'2-digit',month:'short'})+' ('+rd.length+' j)')
            return (
